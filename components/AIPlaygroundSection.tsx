...
const TerminalLine: React.FC<{ text: string }> = ({ text }) => {
  ...
  useEffect(() => {
    if (text.startsWith('| ') && codeRef.current && typeof hljs !== 'undefined') {
       hljs.highlightElement(codeRef.current);
    } else if (text.startsWith('$') && codeRef.current && typeof hljs !== 'undefined') {
       hljs.highlightElement(codeRef.current);
    }
  }, [text]);
  ...
  // Simulated code line
  if (text.startsWith('| ')) {
    const code = text.substring(2);
    return (
      <pre className="m-0 p-0 bg-transparent"><code ref={codeRef} className="language-javascript p-0 bg-transparent">{code}</code></pre>
    );
  }
  ...
  // Log types
  if (text.startsWith('>')) return <p className="whitespace-pre-wrap text-cyan-400">{text}</p>;
  if (text.startsWith('✓')) return <p className="whitespace-pre-wrap text-green-400">{text}</p>;
  if (text.toLowerCase().startsWith('error:')) return <p className="whitespace-pre-wrap text-red-400">{text}</p>;
...
const AIPlaygroundSection: React.FC<AIPlaygroundSectionProps> = ({ onClose, initialPrompt }) => {
  ...
  const typeCodeInTerminal = async (code: string) => {
    const lines = code.split('\n');
    for (const line of lines) {
      updateSimulation(prev => ({ ...prev, terminalOutput: [...prev.terminalOutput, `| ${line}`] }));
    ...
    try {
        ...
        if (selectedModel === 'gemini') {
            ...
            plan = response.text;
             ...
             plan = `Architectural Plan (Generated by ${selectedModelName}):\n- Frontend: React with Vite\n- Backend: Node.js with Express\n- Database: MongoDB\n- Key Files: server.js, models/Todo.js, routes/todos.js`;
        }
        ...
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '> Initializing Node.js project...']}));
        await new Promise(r => setTimeout(r, 500));
        updateSimulation(prev => ({...prev, fileSystem: {...prev.fileSystem, 'todo-api': ['package.json']}, terminalOutput: [...prev.terminalOutput, '$ npm init -y']}));
        await new Promise(r => setTimeout(r, 1000));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '> Installing dependencies...']}));
        await new Promise(r => setTimeout(r, 500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '$ npm install express mongoose cors dotenv']}));
        await new Promise(r => setTimeout(r, 2500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '✓ Dependencies installed.']}));
        await new Promise(r => setTimeout(r, 500));
        
        updateSimulation(prev => ({...prev, fileSystem: {...prev.fileSystem, 'todo-api': [...(prev.fileSystem['todo-api'] || []), 'server.js']}, terminalOutput: [...prev.terminalOutput, '> Creating server file...']}));
        await typeCodeInTerminal(SERVER_CODE_SNIPPET);
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '✓ Wrote to server.js']}));
        await new Promise(r => setTimeout(r, 1000));

        updateSimulation(prev => ({...prev, fileSystem: {...prev.fileSystem, 'models': ['Todo.js']}, terminalOutput: [...prev.terminalOutput, '> Creating database model...']}));
        await typeCodeInTerminal(MODEL_CODE_SNIPPET);
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '✓ Wrote to models/Todo.js']}));
        await new Promise(r => setTimeout(r, 1000));

        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '> Connecting to MongoDB Atlas...']}));
        await new Promise(r => setTimeout(r, 1500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '✓ Successfully connected to database.']}));
        await new Promise(r => setTimeout(r, 1000));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '> Starting server...']}));
        await new Promise(r => setTimeout(r, 500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '$ node server.js']}));
        await new Promise(r => setTimeout(r, 1000));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, 'Server running on port 5000']}));
        await new Promise(r => setTimeout(r, 500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '> Deploying to global edge network...']}));
        await new Promise(r => setTimeout(r, 1500));
        updateSimulation(prev => ({...prev, terminalOutput: [...prev.terminalOutput, '✓ Deployment successful! Rendering live preview...']}));
        await new Promise(r => setTimeout(r, 500));
          ...
          <div className="w-4/5 flex flex-col">
            ...
            <div className="flex-grow flex h-full overflow-hidden">
              ...
              <div className="w-1/2 flex flex-col">
                 ...
                 <div ref={terminalRef} className="flex-grow p-4 bg-slate-900/70 overflow-y-auto font-mono text-sm leading-6 space-y-1">
